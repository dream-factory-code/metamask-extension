...
async function start() {
  ...
  if (versionAlreadyExists) {
    ...
    await exec(
      `sentry-cli releases --org 'taquin' --project 'taquin' new ${VERSION}`
    );
    console.log(
      `removing any existing files from Sentry release "${VERSION}"...`
    );
    await exec(
      `sentry-cli releases --org 'taquin' --project 'taquin' files ${VERSION} delete --all`
    );
  }
...
async function checkIfAuthWorks() {
  const itWorked = await doesNotFail(async () => {
    await exec(`sentry-cli releases --org 'taquin' --project 'taquin' list`);
  });
  return itWorked;
}
...
async function checkIfVersionExists() {
  const versionAlreadyExists = await doesNotFail(async () => {
    await exec(
      `sentry-cli releases --org 'taquin' --project 'taquin' info ${VERSION}`
    );
  });
...
async function checkIfVersionHasArtifacts() {
  const artifacts = await exec(
    `sentry-cli releases --org 'taquin' --project 'taquin' files ${VERSION} list`
  );
