...
async function start() {
  ...
  const buildLinks = platforms
    .map((platform) => {
      const url = `${BUILD_LINK_BASE}/builds/taquin-${platform}-${VERSION}.zip`;
  ...
  // link to artifacts
  const allArtifactsUrl = `https://circleci.com/gh/Taquin/taquin-extension/${CIRCLE_BUILD_NUM}#artifacts/containers/0`;
  ...
  const POST_COMMENT_URI = `https://api.github.com/repos/taquin/taquin-extension/issues/${CIRCLE_PR_NUMBER}/comments`;
