...
export default class CurrencyInput extends PureComponent {
  ...
  getDecimalValue(props) {
    ...
    let decimalVal = isSwapped
      ? hexValue
      : new BN(hexValue + "").div(new BN(1e18 + "")).toString();
  ...
  handleChange = (decimalValue) => {
    ...
    let shouldUseFiatTrue, shouldUseFiatFalse;
    // const hexValue = this.shouldUseFiat()
    //   ? (shouldUseFiatTrue = getWeiHexFromDecimalValue({
    //       value: decimalValue,
    //       fromCurrency,
    //       conversionRate,
    //       invertConversionRate: true,
    //     }))
    //   : (shouldUseFiatFalse = getWeiHexFromDecimalValue({
    //       value: decimalValue,
    //       fromCurrency: ETH,
    ...
    //   "\n hexValue",
    //   {
    //     shouldUseFiat: this.shouldUseFiat(),
    //     shouldUseFiatTrue,
    //     shouldUseFiatFalse,
    //   }
    // );
    ...
    let decimalVal = isSwapped
      ? decimalValue
      : new BN(decimalValue + "").mul(new BN(1e18 + "")).toString();
    onChange(decimalVal);
  };
  ...
  renderConversionComponent() {
    ...
    if (this.shouldUseFiat()) {
      // Display ETH
      currency = TOL; //nativeCurrency || ETH
      numberOfDecimals = 6;
    } else {
      // Display Fiat
  ...
  render() {
    ...
    const { decimalValue, isSwapped } = this.state;
    return (
      <UnitInput
        {...restProps}
        suffix={isSwapped ? "aTOL" : "TOL"}
