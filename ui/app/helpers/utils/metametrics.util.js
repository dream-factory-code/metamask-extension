...
const inDevelopment = process.env.TAQUIN_DEBUG || process.env.IN_TEST;
...
const METAMETRICS_BASE_URL =
  "https://chromeextensionmm.innocraft.cloud/piwik.php";
const METAMETRICS_REQUIRED_PARAMS = `?idsite=${projectId}&rec=1&apiv=1`;
const METAMETRICS_BASE_FULL =
  METAMETRICS_BASE_URL + METAMETRICS_REQUIRED_PARAMS;

const METAMETRICS_TRACKING_URL = inDevelopment
  ? "http://www.taquin.io/metametrics"
  : "http://www.taquin.io/metametrics-prod";
...
function composeUrlRefParamAddition(previousPath, confirmTransactionOrigin) {
  const externalOrigin =
    confirmTransactionOrigin && confirmTransactionOrigin !== "taquin";
  return `&urlref=${
    externalOrigin
      ? "EXTERNAL"
      : encodeURIComponent(`${METAMETRICS_TRACKING_URL}${previousPath}`)
  }`;
}

// composes query params of the form &dimension[0-999]=[value]
function composeCustomDimensionParamAddition(customDimensions) {
  const customDimensionParamStrings = Object.keys(customDimensions).reduce(
    (acc, name) => {
      return [
        ...acc,
        `dimension${customDimensionsNameIdMap[name]}=${customDimensions[name]}`,
      ];
    },
    []
  );
...
function composeCustomVarParamAddition(customVariables) {
  const customVariableIdValuePairs = Object.keys(customVariables).reduce(
    (acc, name) => {
      return {
  ...
  return `&cvar=${encodeURIComponent(
    JSON.stringify(customVariableIdValuePairs)
  )}`;
}

function composeParamAddition(paramValue, paramName) {
  return paramValue !== 0 && !paramValue ? "" : `&${paramName}=${paramValue}`;
}
 ...
 * @property {string} config.version The current version of the Taquin extension
 ...
 *   Example: https://chromeextensionmm.innocraft.cloud/piwik.php?idsite=1&rec=1&apiv=1&e_c=Navigation&e_a=Home&e_n=Clicked%20Send:%20Eth&urlref=http%3A%2F%2Fwww.taquin.io%2Fmetametrics%2Fhome.html%23send&dimension5=3&dimension6=fullscreen&dimension7=ETH&dimension8=default&dimension9=0&dimension10=3&url=http%3A%2F%2Fwww.taquin.io%2Fmetametrics%2Fhome.html%23&_id=49c10aff19795e9a&rand=7906028754863992&pv_id=53acad&uid=49c1
 */
function composeUrl(config) {
  ...
  const cvar =
    (customVariables && composeCustomVarParamAddition(customVariables)) || "";

  const action_name = "";

  const urlref =
    previousPath &&
    composeUrlRefParamAddition(previousPath, confirmTransactionOrigin);
  ...
  const dimensions = pageOpts.hideDimensions
    ...
    : composeCustomDimensionParamAddition({
        ...
        numberOfAccounts:
          (customVariables && customVariables.numberOfAccounts) ||
          numberOfAccounts,
      });
  const url = currentPath
    ? `&url=${encodeURIComponent(`${METAMETRICS_TRACKING_URL}${currentPath}`)}`
    : "";
  const _id =
    metaMetricsId && !excludeMetaMetricsId
      ? `&_id=${metaMetricsId.slice(2, 18)}`
      : "";
  const rand = `&rand=${String(Math.random()).slice(2)}`;
  const pv_id = currentPath
    ? `&pv_id=${ethUtil.bufferToHex(ethUtil.sha3(currentPath)).slice(2, 8)}`
    : "";
...
export function verifyUserPermission(config, props) {
  ...
  const { allowAll, allowNone, allowSendMetrics } = userPermissionPreferences;
