...
const unapprovedTxsSelector = (state) => state.taquin.unapprovedTxs;
const unapprovedMsgsSelector = (state) => state.taquin.unapprovedMsgs;
...
const networkSelector = (state) => state.taquin.network;
...
export const unconfirmedTransactionsHashSelector = createSelector(
  ...
  (
    ...
    const filteredUnapprovedTxs = Object.keys(unapprovedTxs).reduce(
      (acc, address) => {
        const { taquinNetworkId } = unapprovedTxs[address];
        const transactions = { ...acc };

        if (taquinNetworkId === network) {
...
const unapprovedMsgCountSelector = (state) => state.taquin.unapprovedMsgCount;
const unapprovedPersonalMsgCountSelector = (state) =>
  state.taquin.unapprovedPersonalMsgCount;
const unapprovedDecryptMsgCountSelector = (state) =>
  state.taquin.unapprovedDecryptMsgCount;
const unapprovedEncryptionPublicKeyMsgCountSelector = (state) =>
  state.taquin.unapprovedEncryptionPublicKeyMsgCount;
const unapprovedTypedMessagesCountSelector = (state) =>
  state.taquin.unapprovedTypedMessagesCount;
...
export const unconfirmedTransactionsCountSelector = createSelector(
  ...
  (
    ...
    const filteredUnapprovedTxIds = Object.keys(unapprovedTxs).filter(
      (txId) => {
        const { taquinNetworkId } = unapprovedTxs[txId];
        return taquinNetworkId === network;
      }
    );
...
export const currentCurrencySelector = (state) => state.taquin.currentCurrency;
export const conversionRateSelector = (state) => state.taquin.conversionRate;
  ...
  (params, tokenDecimals) => {
    ...
    if (params && params.length) {
      ...
      const valueParam = params.find(
        (param) => param.name === TOKEN_PARAM_VALUE
      );
...
export const approveTokenAmountAndToAddressSelector = createSelector(
  ...
  (params, tokenDecimals) => {
    ...
    if (params && params.length) {
      toAddress = params.find(
        (param) => param.name === TOKEN_PARAM_SPENDER
      ).value;
      const value = Number(
